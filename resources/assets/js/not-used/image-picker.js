        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                for (let i = 0; i < result.length; i++) {
                    images.append(' <div style="background-color:' + result[i].color + ';background-image: url(' + result[i].path + '.' + result[i].extension + ')" class="image-picker__item" data-name="' + result[i].name + '" data-path="' + result[i].path_thumbnail + '" data-id="' + result[i].id + '"></div>');
                }
